...
def time_series(args):
    '''Tests Accuracies with respect to different Times
    '''
    ...
    for time in range(300,1001,50):
        ...
        cnn_transformer_model = create_cnn_transformer_model(input_shape=(int(time/2), 1, 22), time=time)
        # Compiling the model
        cnn_transformer_model.compile(loss='categorical_crossentropy',
                        optimizer=cnn_optimizer,
                        metrics=['accuracy'])

        # Training and validating the model
        cnn_transformer_model_results = cnn_transformer_model.fit(X_train_set,
                    y_train_set,
                    batch_size=64,
                    epochs=epochs,
                    validation_data=(X_valid_set, y_valid_set), verbose=1)
        
        cnn_score = cnn_transformer_model.evaluate(X_valid_set, y_valid_set, verbose=0)
        accuracies.append(cnn_score[1])
        print('Validation accuracy of the CNN model with time {0}: {1}'.format(time,cnn_score[1]))

    plt.figure()
    plt.plot(range(300,1001,50), accuracies)
    plt.title('Test Accuracy of CNN as a function of Time')
    plt.xlabel('Time')
...
if __name__ == '__main__':
    ...
    # Model parameters
    learning_rate = 4e-4 # back to 4e-4
    epochs = 1 # change back to 200
    cnn_optimizer = keras.optimizers.Adam(lr=learning_rate)
    ...
    for time in range(300,1001,50):
        ...
        cnn_score = basic_cnn_model.evaluate(X_valid_set, y_valid_set, verbose=0)
        accuracies.append(cnn_score[1])
        print('Validation accuracy of the CNN model with time {0}: {1}'.format(time,cnn_score[1]))
